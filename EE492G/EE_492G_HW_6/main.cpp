...
void curveHist(vector<float> curvature){
	...
	int hist_h = 500;
	int hist_w = 800;
	int max = (int)*max_element(curvature.begin(),curvature.end());
	Size hist_size;
	hist_size.height = hist_h;
	hist_size.width = hist_w;
	Mat Hist = Mat::zeros(hist_size, CV_32S);
	int bin_w = round((double)hist_w/curvature.size());

	cout << "H: " << hist_h << endl;
	cout << "max: " << max << endl;
	cout << "max-10: " << (max-10)  << endl;
	cout << "(max-10)/max: " << (long)((max-10)/max) << endl;
	cout << "pos: " << hist_h * 0.1 << endl;//(max-10)/max << endl;


	// draw histogram
	for(size_t i = 1; i < curvature.size(); i++){
		line(	Hist, Point(bin_w * (i-1), (int)( hist_h * (curvature[i-1]/max) ) ), Point(bin_w * (i), (int)( hist_h * (curvature[i]/max) ) ),
				Scalar(255,0,0),
				2, 8);
	}
...
void curvature(Mat src)
{
	...
	for (double sig = 1.0; sig <= 1.0; sig++){
		...
		for (size_t i = 0; i< contours.size(); i++)
		{
			...
			//k[i] = kl;
			cout << "Max: " << *max_element(kl.begin(), kl.end()) << endl;
			curveHist(kl);
		}
